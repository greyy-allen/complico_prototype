...
const selectVariants = cva(
  " flex h-full w-auto items-center justify-between px-3 py-2 placeholder:text-muted-foreground focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
  {
    variants: {
      ...
      size: {
        xl: "h-[44px] px-3 text-[14px]",
        md: "h-[30px] pl-3.5 pr-1.5 text-[14px]",
      },
      shape: {
        round: "rounded-[5px]",
      },
...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof Trigger>, SelectProps>(
  (
    { variant = "fill", shape, size = "md", colorScheme = "white_A700", className, children, indicator, ...rest },
    ref,
  ) => {
    return (
...
>(({ className, children, labelIcon, ...props }, ref) => (
  <Item
    ref={ref}
    className={cn(
      "hover:bg-[#f1f1ff] hover:text-[#000000] relative flex w-full cursor-default select-none items-center py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className,
    )}
    {...props}
  >
